...
def cleanse_student_table(df):
    ...
    Parameters:
        df (DataFrame): `student` table from `code.db`
...
def cleanse_career_path(df):
    """
    Cleanse the `code_courses` table according to the discoveries made in the writeup

    Parameters:
        df (DataFrame): `code_courses` table from `code.db`
